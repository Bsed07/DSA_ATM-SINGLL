...
class Account_List{
    ...
    Node* Login(){
        ...
        cin.ignore(1000, '\n');
        e_Pin = GetMaskedPin();
        string e_E_p = encryptPin(e_Pin);

        if(e_E_p != found->data.pin){
            cout << "Incorrect PIN.\n";
            return NULL;
        }
        cout << "Welcome, " << found->data.acc_Name << "!\n";
        return found;
    }

    void Bal_Inquiry(Node* acc){
        cout << fixed << setprecision(2);
        cout << "Current Balance: ₱ " << acc->data.balance << endl;
    }

    void Withdraw(Node* acc){
        double amount;
        cout << "Enter the amount that you want to withdraw: ";
        cin >> amount;

        if(amount <= 0){
            cout << "Minumum withdrawal amount is ₱1.00";
            return;
        }

        if(amount > acc->data.balance){
            cout << "Your funds are less than the amount entered!";
            return;
        }

        acc->data.balance -= amount;
        cout << "Succesfull withdrawal.\n\n" 
        << "New Balance: " 
        << fixed << setprecision(2) << acc->data.balance << endl;
    }

    void Deposit(Node* acc){
        double amount;
        cout << "Enter the amount that you want to deposit: ";
        cin >> amount;

        if(amount <= 0){
            cout << "Minumum deposit amount is ₱1.00";
            return;
        }
        ...
        switch (choice) {
            ...
            case 3:
                ...
                do{
                    ...
                    switch(ch){
                        case 1: ATM.Deposit(loggedIN); break;
                        case 2: ATM.Withdraw(loggedIN); break;
                        case 3: ATM.Bal_Inquiry(loggedIN); break;
                        case 4: cout << "Logging out..."; this_thread::sleep_for(chrono::seconds(2)); break;
                    }
                }while(ch != 4);
